...
public partial class RussianAnalyzer
{
    ...
    [
        "без", "близ", "в", "во", "для", "до", "за", "из", "изо", "к", "ко",
        "на", "над", "о", "об", "обо", "от", "ото", "перед", "передо", "по",
        "под", "подо", "при", "про", "с", "со", "у", "через", "чрез"
    ];
    
    private static readonly HashSet<string> Conjunctions =
    [
        "и", "а", "но", "да", "или", "либо", "то", "как", "что", "чтобы",
        "если", "когда", "потому", "так", "же"
    ];
    ...
    [
        "человек", "мужчина", "женщина", "мальчик", "девочка", "ребёнок", "ребенок",
        "кот", "кошка", "собака", "пёс", "пес", "попугай", "рыба", "птица",
        "друг", "подруга", "парень", "девушка", "учитель", "врач",
        "студент", "школьник", "папа", "мама", "дедушка", "бабушка",
        "брат", "сестра", "сын", "дочь", "дядя", "тётя", "юноша"
    ];
    ...
    public static string[] Tokenize(string phrase)
    {
        ...
        var tokens = MyRegex().Split(phrase)
    ...
    public static WordInfo Analyze(string word)
    {
        ...
        if (Prepositions.Contains(word)) { info.PartOfSpeech = PartOfSpeech.Preposition; return info; }
        if (Conjunctions.Contains(word)) { info.PartOfSpeech = PartOfSpeech.Conjunction; return info; }
    ...
    private static void DeterminePartOfSpeech(WordInfo info)
    {
        ...
        if (Vowels.Contains(last) || last == 'ь' || last == 'й' || 
            (IsConsonant(last) && last != 'ь' && last != 'й' && last != 'ъ'))
        {
            ...
            if ((w.EndsWith("ый") || w.EndsWith("ий") || w.EndsWith("ой") ||
                 w.EndsWith("ая") || w.EndsWith("яя") ||
                 w.EndsWith("ое") || w.EndsWith("ее") ||
                 w.EndsWith("ые") || w.EndsWith("ие")) && w.Length > 3)
            {
    ...
    {
        if (info.PartOfSpeech != PartOfSpeech.Noun) return;
        ...
        else if (w.EndsWith("жь") || w.EndsWith("шь") || 
                 w.EndsWith("чь") || w.EndsWith("щь"))
        {
            info.Gender = GrammaticalGender.Feminine;
        }
    ...
    private static void DetermineAnimacy(WordInfo info)
    {
        ...
        var w = info.Original;
        if (w.EndsWith("тель") || w.EndsWith("ист") || 
            w.EndsWith("ец") || w.EndsWith("щик") ||
            w.EndsWith("чик") || w.EndsWith("арь") ||
            w.EndsWith("ак") || w.EndsWith("як") ||
            w.EndsWith("лог") || w.EndsWith("вед") ||
            w.EndsWith("ёр") || w.EndsWith("ер") ||
            w.EndsWith("атор") || w.EndsWith("итор") ||
            w.EndsWith("ик") || w.EndsWith("ник") ||
            w.EndsWith("ун") || w.EndsWith("ок"))
        {
            info.Animacy = Animacy.Animate;
        }
        
        // Женские профессии и люди
        if (info.Gender == GrammaticalGender.Feminine && 
            (w.EndsWith("ица") || w.EndsWith("ница") || 
             w.EndsWith("чица") || w.EndsWith("щица") ||
             w.EndsWith("ша") || w.EndsWith("иха")))
        {
            info.Animacy = Animacy.Animate;
        }
    }
    ...
    private static void DetermineSoftHissing(WordInfo info)
    {
        if (info.PartOfSpeech != PartOfSpeech.Noun && 
            info.PartOfSpeech != PartOfSpeech.Adjective) return;
        
        var w = info.Original;
        if (w.Length == 0) return;
        
        char last = w[^1];
        if (Hissing.Contains(last)) info.IsHissing = true;
    ...
    {
        if (info.PartOfSpeech != PartOfSpeech.Noun) return info.Original;
        
        var w = info.Original;
        if (w.Length <= 2) return w; // короткие слова не усекаем
    ...
    public static string GetDeclensionStem(WordInfo info)
    {
        ...
        if (w.EndsWith("ец") && w.Length > 3 && info.PartOfSpeech == PartOfSpeech.Noun && info.Gender == GrammaticalGender.Masculine)
        {
            return w[..^2] + "ц"; // боец→бойц, певец→певц, купец→купц
        }
        if (w.EndsWith("ок") && w.Length > 3 && info.PartOfSpeech == PartOfSpeech.Noun && info.Gender == GrammaticalGender.Masculine)
        {
